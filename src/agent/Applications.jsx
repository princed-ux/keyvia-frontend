...
const Applications = () => {
  ...
  const fetchApplications = async () => {
    try {
      // Don't set loading(true) here to avoid flashing screen on auto-refresh
      const res = await client.get('/api/applications/agent');
      setApplications(res.data);
    } catch (err) {
      console.error("Error fetching applications:", err);
      // Optional: Only toast on initial error, not background refresh
  ...
  // ✅ 3. Use the Hook (Replaces the old useEffect)
  // This runs on mount AND whenever a relevant socket event hits.
  ...
  return (
    <div className={style.pageContainer}>
      ...
      <div className={style.header}>
        <div>
            <h1>Applications</h1>
            <p>Review and manage offers from interested buyers.</p>
        </div>
