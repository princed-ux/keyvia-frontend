...
const Profile = () => {
  ...
  const renderAvatar = () => {
    ...
    return (
        <div className={styles.avatar} onClick={() => setShowImageModal(true)} style={{backgroundColor: bgColor, display: 'flex', alignItems: 'center', justifyContent: 'center', color: 'white', fontSize: '40px', fontWeight: '600', cursor: 'pointer', letterSpacing: '1px'}}>
            {initials}
        </div>
    );
  };
            ...
            <div className={styles.guidelineModal}>
                ...
                <div className={styles.guidelineActions}>
                    ...
                    <button className={styles.confirmBtn} onClick={handleProceedToUpload}>
                        <UploadCloud size={18} /> I Understand, {(avatarPreview || remoteAvatarUrl) ? "Change Photo" : "Select Photo"}
                    </button>
                </div>
            </div>
        </div>
